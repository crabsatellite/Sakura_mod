...
public class IronChainReachabilityTest {
    ...
    private static Set<Item> computeReachable(Set<Item> seed, RecipeManager rm, HolderLookup.Provider ra) {
        Set<Item> reachable = new HashSet<>(seed);
        Collection<RecipeHolder<?>> all = rm.getRecipes();
        int prev = -1;
    ...
    public static void every_iron_chain_intermediate_is_both_produced_and_consumed(GameTestHelper helper) {
        ...
        HolderLookup.Provider ra = helper.getLevel().registryAccess();
        Collection<RecipeHolder<?>> all = rm.getRecipes();
